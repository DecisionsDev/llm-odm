...
from prompts import FORMAT_RESPONSE
import json
@staticmethod
...
@staticmethod
def format_to_markdown(data,llm):
    markdown_text = ""
    # friendlyName={
    ...
    print(str(data))
    for msg in data['loan']['messages']:
        print("Message"+msg)
    prompt_template = "Tell me a {adjective} joke"
    prompt = PromptTemplate(
        input_variables=["decisionresponse"], template=FORMAT_RESPONSE
    )
    decisionresponse=" approved : "+ str(data['loan']['approved'])+"\n messages : "+  str(data['loan']['messages'])
  
    prompt_and_model = prompt | llm
    output = prompt_and_model.invoke({"decisionresponse": decisionresponse})  
...
@staticmethod
def formatDecisionResponse(message,llm):
    if "error" in message:
        return message['error']
    return format_to_markdown(message,llm)
